...
if [ -z "$USER_POOL_ID" ]; then
    echo "Error: USER_POOL_ID is not set in the .env file."
    exit 1
fi
...
test_users=$(aws cognito-idp list-users --user-pool-id $USER_POOL_ID --region $AWS_REGION --query "Users[?starts_with(Username, 'testuser_')].Username" --output text)

# Loop through each test user and delete them
for username in $test_users
do
  echo "Deleting user: $username"
  output=$(aws cognito-idp admin-delete-user --user-pool-id $USER_POOL_ID --username $username --region $AWS_REGION 2>&1)
  exit_code=$?
...
remaining_users=$(aws cognito-idp list-users --user-pool-id $USER_POOL_ID --region $AWS_REGION --query "Users[?starts_with(Username, 'testuser_')].Username" --output text)
